...
class _ChatCompanyScreenState extends State<ChatCompanyScreen> {
  ...
  Map<String, dynamic> _normalizeMessage(Map m) {
    ...
    // Legacy support: isSystem/type-like hints.
    ...
    // If senderId is null, treat as SYSTEM.
    ...
    // Text canonical
    final text = (normalized['text'] ?? normalized['message'] ?? '').toString();
    normalized['text'] = text;

    // createdAt canonical (best effort)
    ...
    // Normalize optional sender fields if present.
    ...
    // If server sent isMine, keep it (could be false).
  ...
  bool _computeIsMe(Map m) {
    ...
    // System messages are neither "me" nor "them".
    final type = m['type']?.toString().toUpperCase();
    if (type == 'SYSTEM' || m['senderId'] == null) return false;

    // Legacy/optimistic flags (kept for backwards compatibility)
  ...
  Future<void> _send() async {
    ...
    try {
      ...
      // Sync with server so IDs/sender info are canonical.
      if (mounted) {
        await _loadMessages();
      }
    } catch (e) {
